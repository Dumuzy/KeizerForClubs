...
namespace KeizerForClubs
{
  public class frmPairingManual : Form
  {
    ...
    public frmPairingManual() => this.InitializeComponent();
    ...
    private bool fCheckPairings()
    {
      if (this.lstNames.Items.Count > 1)
        return false;
      for (int index = 0; index < this.grdPaarungen.Rows.Count; ++index)
      {
        if (this.grdPaarungen.Rows[index].Cells[0].Value == null && this.grdPaarungen.Rows[index].Cells[1].Value != null || this.grdPaarungen.Rows[index].Cells[0].Value != null && this.grdPaarungen.Rows[index].Cells[1].Value == null)
          return false;
      }
      return true;
    }
